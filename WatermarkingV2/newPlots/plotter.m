...
wlens = 0:10:100;
newUtil = [100-utilityLoss(1,:); 100-attackUtilityLoss(1,:); 100*ones([1 11])];
newUtil2 = [utilityLoss(2,:); attackUtilityLoss(2,:); 0.5*ones([1 11])];
figure
a = plot(wlens,newUtil);
grid on
%title('Impact of Watermark Length on Kinship Coefficient (\phi)') %%Colab can be changed here
title('Impact of Watermark Length on Utility (Entropy Based)')
xlabel('Watermark Length')
%ylabel('Kinship Coefficient (\phi)')
ylabel('% of Preserved Utility')
legend('Watermarked Data', 'Attacker Data', 'Original Data')
a(1).Marker = 'o';
a(2).Marker = '+';
a(3).Marker = 'x';
%set([x y],'interpreter','latex','fontSize','bold')
